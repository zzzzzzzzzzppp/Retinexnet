...
#DecomNet模型，对应论文中的Decomposition
#输入低/正常光照图像对并共享网络参数，得到低光照图像的反射分量和光照分量
#模型架构--包含三个主要层：详见附录中一
class DecomNet(nn.Module):
    def __init__(self, channel=64, kernel_size=3):
        ...
        self.net1_conv0 = nn.Conv2d(4, channel, kernel_size * 3,
                                    padding=4, padding_mode='replicate')    #详见附录中2，后面代码中形状类似这样的都是一样的概念
        # Activated layers!
...
#RelightNet模型，对应论文中的adjustment的下半部分enhance-net
#对低照度的光照分量进行处理，整体上是一个encoder-decoder架构，同时引入了多尺度的连接。
#模型架构--详见附录中二
...
class RetinexNet(nn.Module):
    ...
    def forward(self, input_low, input_high):
        ...
        #输出tensor结果
        #detach() 表示将 tensor 从计算图中分离出来，不再进行梯度计算，cpu() 表示将 tensor 转移到 CPU 上运行
    ...
    #平均梯度计算函数 ave_gradient，用于计算输入 tensor 在水平或竖直方向上的平均梯度信息。
    def ave_gradient(self, input_tensor, direction):
        return F.avg_pool2d(self.gradient(input_tensor, direction),
                            kernel_size=3, stride=1, padding=1)

    #平滑计算函数 smooth，用于计算输入图像在亮度方向上的平滑误差
    def smooth(self, input_I, input_R):
        #对输入的反射分量图像进行灰度化处理，采用常见的加权平均法，其中 0.299、0.587 和 0.114 分别是 R、G 和 B 通道的亮度系数；
        input_R = 0.299 * input_R[:, 0, :, :] + 0.587 * input_R[:, 1, :, :] + 0.114 * input_R[:, 2, :, :]
        input_R = torch.unsqueeze(input_R, dim=1)
        #将亮度分量图像在水平和竖直方向上的梯度信息与反射分量图像在相应方向上的指数函数进行逐元素相乘。由于指数函数值是在 (0, 1] 区间内的，因此会对梯度信息进行一定程度的抑制或放大，从而达到平滑的目的；
        return torch.mean(self.gradient(input_I, "x") * torch.exp(-10 * self.ave_gradient(input_R, "x")) +
                          self.gradient(input_I, "y") * torch.exp(-10 * self.ave_gradient(input_R, "y")))

    #评估函数 evaluate，用于在训练过程中对模型进行评估
    ...
    #保存函数 save，用于将训练好的模型参数保存到指定的目录中
    ...
    #加载函数 load，用于从指定的目录中加载已经训练好的模型参数
    ...
    #训练函数 train，用于训练指定阶段（分解或重照）下的模型。
    #详见附录三（4）
    def train(self,
        ...
        for idx in range(len(test_low_data_names)):
            ...
            if save_R_L:
                cat_image = np.concatenate(
                    [input.detach().numpy(), result_1.detach().numpy(), result_2.detach().numpy(),
                     result_3.detach().numpy(), result_4.detach().numpy()], axis=2)
            else:
                cat_image = result_4.detach().numpy()
